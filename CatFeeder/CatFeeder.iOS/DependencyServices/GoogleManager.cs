...
namespace CatFeeder.iOS.DependencyServices
{
    public class GoogleManager : NSObject, IGoogleManager, ISignInDelegate
    {
        private Action<CatFeeder.Models.Google.GoogleUser, string> _onLoginComplete;
        ...
        public void Login(Action<CatFeeder.Models.Google.GoogleUser, string> OnLoginComplete)
        {
            try
            {
            ...
            {
                MessagingCenter.Send<string>(ex.Message, "NoInternet");
            }
        ...
        public void DidSignIn(SignIn signIn, Google.SignIn.GoogleUser user, NSError error)
        {
            if (user != null && error == null)
                _onLoginComplete?.Invoke(new Models.Google.GoogleUser()
                {
                    Name = user.Profile.Name,
                    AccessToken = user.Authentication.AccessToken,
                    ExpirationDate = user.Authentication.AccessTokenExpirationDate.ToString(),
        ...
        public void DidDisconnect(SignIn signIn, Models.Google.GoogleUser user, NSError error)
        {
